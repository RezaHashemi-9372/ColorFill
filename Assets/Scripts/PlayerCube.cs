...
public class PlayerCube : MonoBehaviour
{
    ...
    private void Update()
    {
        if (shouldMove)
        {
            ...
            {
                if (frontObj && frontObj.GetComponent<Cube>().boxtype == Cube.colorType.unfilled)
                {
                    if (isMoving == false)
                    {
                        isMoving = true;
                    ...
                    {
                        startPoint = frontObj.transform.position;
                    }
                }
                else if (frontObj.GetComponent<Cube>().boxtype == Cube.colorType.highlight)
    ...
    public void Move(Vector3 dir)
    {
        ...
        {
            if (rayObj.GetComponent<Cube>().boxtype == Cube.colorType.highlight)
            {
                //gameMode.Restart();
            }
